...
class utm {
	...
	public function add()
	{
		Event::add('ushahidi_action.report_form_admin', array($this, '_insert_UTM'));		
		Event::add('ushahidi_action.report_form', array($this, '_insert_UTM'));
		
		Event::add('ushahidi_action.report_meta', array($this,'_show_UTM'));
	}
	
	/**
	 * For editing the UTM date
	 */
	public function _insert_UTM()
	{
		// Load the View
		$form = View::factory('utm/utm_edit');
		$form->render(TRUE);
	}//end method
	
	/**
	* For showing the UTM coordinates to the user
	*/
	public function _show_UTM()
	{
		//get report info
		...
		//convert lat/lon
		$point = new gPoint();
		$point->setLongLat($lon, $lat);
		$point->convertLLtoTM();
		
		
		
		$form = View::factory('utm/utm_show');
		$form->zone = $point->Z();
		$form->easting = $point->E();
		$form->northing = $point->N();
		$form->render(TRUE);
	}
